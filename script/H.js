	...
	if ( typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
		...
		factory(window['H'] = {
			...
			isEmail : function(email) {
				return (/^[A-Za-z1-9]\w+@[A-Za-z0-9]+\.[a-z]+$/i).test(email);
			},
			//          	判断是否是password
			...
			//          	判断是否是车牌号
			isCarnumber : function(car) {
				return (/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[A-Z_0-9]{5}$/).test(car);
			},
			//				判断是否是车架号
			isPlate : function(plate){
				return (/^[A-Z0-9]{17}$/).test(plate);
			},
			...
			DEFAULT_CONFIG : {
				...
				closeToWin_CONFIG : {},
				setWinAttr_CONFIG : {},
				openFrame_CONFIG : {//打开新的frame，并设置相关属性
					bounces : true,
					bgColor : "rgba(0,0,0,0)",
					scrollToTop : true,
					vScrollBarEnabled : false,
				...
				openFrameGroup_CONFIG : {//设置打开新的frame-group，并设置相关的属性
					scrollEnabled : true,
					rect : {
						x : 0,
						y : 0,
				...
				openPopoverWin_CONFIG : {//ipad上的弹出窗口，弹出后的窗口不允许操作
					width : 540,
					height : 620,
					bgColor : 'rgba(0,0,0,0)',
					scrollToTop : true,
				...
				openSlidLayout_CONFIG : {//打开侧滑窗口以及侧滑位置
					type : "left"
				},
				openDrawerLayout_CONFIG : {//打开一个侧滑弹出窗口
					bounces : false,
					bgColor : "rgba(0,0,0,0)",
					scrollToTop : true,
					vScrollBarEnabled : false,
			...
			dateTip : function(element, okCallback, cancelCallback) {
				...
				var itemHeight = 45;
				// 默认下移两个列表项目高度
				that.cssText(that.Ds('.H-select-options .H-select-items'), "transform:translateY(" + (5 * itemHeight) + "px);-webkit-transform:translateY(" + (2 * itemHeight) + "px); transition:transform 0.3s;-webkit-transition:transform 0.3s;");
				// 默认第一个高亮
